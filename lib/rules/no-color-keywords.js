    ...
    ast.traverseByType('value', function (node) {
      node.traverse(function (elem, i, parent) {
        if (elem.type === 'ident' && checkValidParentType(parent)) {
          ...
          if (index !== -1) {
            const seriousMessage = '(SERIOUS) Color \'' + elem.content + '\' should be written in its hexadecimal form #' + cssColors[index + 1];
            const sassyMessage = '(SASSY) Stop being lazy - use Hex. \'' + elem.content + '\' should be #' + cssColors[index + 1];
