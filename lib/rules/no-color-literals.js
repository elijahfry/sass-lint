    ...
    ast.traverseByTypes(['value', 'variable'], function (node, i, parent) {
      ...
      if (node.is('variable') && !parser.options['allow-variable-identifiers']) {
        if (cssColors.indexOf(node.content[0].content) !== -1) {
          const seriousMessage = '(SERIOUS) Color literals should not be used as variable names';
          const sassyMessage = '(SASSY) These are not the color declarations you are looking for';
        ...
        node.forEach(function (valElem) {
          ...
          if (valElem.type === 'color' || cssColors.indexOf(valElem.content) !== -1) {
            if (declarationType === 'ident') {
              const seriousMessage = '(SERIOUS) Color literals such as \'' + checkHexPrefix(valElem.type) + valElem.content + '\' should only be used in variable declarations';
              const sassyMessage = '(SASSY) These are not the color declarations you are looking for';
              result = helpers.addUnique(result, {
                'ruleId': parser.rule.name,
          ...
          // if not a color value or a variable then check if it's a function
            ...
            if (validColorFunctions.indexOf(funcType) !== -1 && declarationType !== 'variable') {
              const seriousMessage = '(SERIOUS) Color functions such as \'' + funcType + '\' should only be used in variable declarations';
              const sassyMessage = '(SASSY) These are not the color declarations you are looking for';
              result = helpers.addUnique(result, {
                'ruleId': parser.rule.name,
                'line': valElem.start.line,
            ...
            // if rgba usage is allowed we need to make sure only variables are being passed to it.
              ...
              const seriousMessage = '(SERIOUS) Only variables or functions must be passed to rgba, literals are restricted';
              const sassyMessage = '(SASSY) These are not the color declarations you are looking for';
              result = helpers.addUnique(result, {
                'ruleId': parser.rule.name,
                'line': valElem.start.line,
                  ...
                  funcContent.forEach(function (funcArgs) {
                    ...
                    if (funcArgs.type !== 'function') {
                      ...
                      if ((funcArgs.type === 'color' || funcArgs.type === 'ident') && (cssColors.indexOf(funcArgs.content) !== -1 || helpers.isValidHex(funcArgs.content))) {
                        const seriousMessage = '(SERIOUS) Color literals such as \'' + checkHexPrefix(funcArgs.type) + funcArgs.content + '\' should not be passed to functions, use variables';
                        const sassyMessage = '(SASSY) These are not the color declarations you are looking for';
                        result = helpers.addUnique(result, {
                          'ruleId': parser.rule.name,
                    ...
                    // if the argument is a function itself
                      ...
                      funcArgs.forEach( function (nestedFuncArgs) {
                        ...
                        if (
                          ...
                          const seriousMessage = '(SERIOUS) Color functions such as \'' + nestedFuncArgs.content + '\' should not be passed to functions, use variables';
                          const sassyMessage = '(SASSY) These are not the color declarations you are looking for';
                          result = helpers.addUnique(result, {
                            'ruleId': parser.rule.name,
          ...
          // are the same as color literals - this is bad
            ...
            valElem.traverse(function (mapVals, idx, mapValsParent) {
              ...
              if (
                ...
                const seriousMessage = '(SERIOUS) Color literals should not be used as map identifiers';
                const sassyMessage = '(SASSY) These are not the color declarations you are looking for';
                result = helpers.addUnique(result, {
                  'ruleId': parser.rule.name,
