          ...
          if (previousVal) {
            if (helpers.isNestable(item.type, previousVal.type, elements, nestableAttributes)) {
              helpers.addUnique(result, {
                ...
                'message': 'With great (attribute nesting) power there must also come great responsibility ' + formatOutput(item.type) + ' should be nested within its parent ' + formatOutput(previousVal.type),
