          ...
          if (previousVal) {
            if (helpers.isNestable(item.type, previousVal.type, elements, nestablePseudo)) {
              helpers.addUnique(result, {
                ...
                'message': 'With great (pseudo nesting) power there must also come great responsibility ' + formatOutput(item.type) + ' should be nested within its parent ' + formatOutput(previousVal.type),
