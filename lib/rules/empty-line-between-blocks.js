      ...
      if (syntax === 'scss') {
        ...
        if (space) {
          if (parser.options.include && !space.space && j !== 1) {
            const seriousMessage = '(SERIOUS) Space expected between blocks';
            const sassyMessage = '(SASSY) Stacking blocks like its Jenga';
            ...
            const seriousMessage = '(SERIOUS) Space not allowed between blocks';
            const sassyMessage = '(SASSY) Stack the blocks like its Jenga';
            result = helpers.addUnique(result, {
              'ruleId': parser.rule.name,
              'line': space.previous.end.line,
              'column': 1,
